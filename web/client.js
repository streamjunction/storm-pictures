...
exports.submit = function (stormJarDir, topologyJar, mainClass, submitUrl, otherArgs, cb) {
  ...
  fs.readdir(stormJarDir, function (err, files) {
    if (err) {
      ...
      var java = cp.spawn("java", [ 
        ...
        mainClass, 
        submitUrl 
      ].concat(otherArgs || []), { stdio: "inherit" });
      
      java.on("error", function (err) {
        console.log("Error while submitting topology"+err);
		cb(err);
      });
